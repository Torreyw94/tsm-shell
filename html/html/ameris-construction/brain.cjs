...
app.post('/api/v1/bridge',async(req,res)=>{try{const r=await axios.post('https://api.groq.com/openai/v1/chat/completions',{model:'llama-3.3-70b-versatile',messages:[{role:'system',content:SYSTEM},...(req.body.messages || [{role:"user",content:req.body.message||"Hello"}])],max_tokens:req.body.max_tokens||1000,temperature:0.4},{headers:{'Authorization':`Bearer ${KEY}`}});const cnt=r.data.choices[0].message.content;res.json({success:true,response:cnt,answer:cnt,content:cnt,choices:r.data.choices});}catch(e){res.status(500).json({error:e.message});}});
