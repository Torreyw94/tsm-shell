...
[ -f "$FILE" ] || { echo "Missing $FILE in $(pwd)"; exit 1; }
...
# remove raw diff marker lines and accidental leading plus signs
cleaned = []
...
# remove an existing honor block if already injected
text = re.sub(
