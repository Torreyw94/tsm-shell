...
mkdir -p backups/dee_polish
cp -f "$FILE" "backups/dee_polish/index.$STAMP.bak"
...
# Replace the prior DEECC block completely so we don't stack behaviors
text = re.sub(
    r"<style>\s*#dee-command-center\{[\s\S]*?</style>\s*<script>\s*window\.DEECC\s*=\s*\{[\s\S]*?setTimeout\(\s*\(\)\s*=>\s*\{\s*window\.DEECC\.load\(\)\.catch\(err => console\.error\('DEECC load failed', err\)\);\s*\},\s*500\s*\);\s*</script>",
    "",
    text,
    flags=re.S
)
...
#dee-command-center .dcc-kpi .val{
  font-size:20px;font-weight:700;color:#ffffff;line-height:1.15;
}
#dee-command-center .dcc-grid{
  display:grid;grid-template-columns:1.18fr 1.02fr .9fr;gap:14px;align-items:start;
}
#dee-command-center .dcc-grid-bottom{
  display:grid;grid-template-columns:1fr 1fr .9fr;gap:14px;align-items:start;margin-top:14px;
}
...
#dee-command-center .dcc-card.dcc-short{
  min-height:300px;
  max-height:300px;
}
...
#dee-command-center .dcc-pre{
  white-space:pre-wrap;line-height:1.72;font-size:13px;
}
#dee-command-center .dcc-list{
  display:grid;gap:8px;
}
#dee-command-center .dcc-item{
  display:flex;justify-content:space-between;gap:12px;padding:10px 10px;border:1px solid rgba(255,255,255,.06);
  border-radius:10px;background:rgba(255,255,255,.02);font-size:13px;cursor:pointer;
}
#dee-command-center .dcc-item:hover{
...
#dee-command-center .dcc-heatmap{
  display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:10px;
}
#dee-command-center .dcc-heat{
  border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:8px 10px;font-size:12px;
}
#dee-command-center .dcc-heat-critical{background:rgba(255,82,82,.12)}
#dee-command-center .dcc-heat-high{background:rgba(255,184,77,.12)}
#dee-command-center .dcc-heat-medium{background:rgba(68,166,255,.12)}
#dee-command-center .dcc-heat-stable{background:rgba(57,214,123,.12)}
@media (max-width: 1380px){
  #dee-command-center .dcc-kpis{grid-template-columns:repeat(3,minmax(0,1fr))}
  #dee-command-center .dcc-grid,
  #dee-command-center .dcc-grid-bottom{grid-template-columns:1fr}
  #dee-command-center .dcc-card,
  #dee-command-center .dcc-card.dcc-short{min-height:unset;max-height:unset}
      ...
      <div class="dcc-grid">
        ...
        <div class="dcc-card">
          ...
          <div class="dcc-actions">
            ...
            <button class="dcc-btn" id="dcc-recovery">Run Recovery Plan</button>
            <button class="dcc-btn" id="dcc-escalate">Escalate Payer</button>
        ...
        <div class="dcc-card">
          ...
          <div class="dcc-head" style="margin-top:12px;">System Pressure Heatmap</div>
          <div class="dcc-heatmap" id="dcc-heatmap"></div>
        </div>
      </div>

      <div class="dcc-grid-bottom">
        <div class="dcc-card dcc-short">
...
${board.execNarrativeNow || '—'}`;
    ...
    document.getElementById('dcc-ranking').innerHTML = rank.map(r => `
      <div class="dcc-item" data-rank-office="${r.office}">
        <div>
    ...
    document.getElementById('dcc-heatmap').innerHTML = rank.map(r => `
      <div class="dcc-heat dcc-heat-${(r.status || 'stable').toLowerCase()}">
        <b>${r.office}</b><br>
  ...
  bindPrimaryButtons(payload){
    ...
    const recovery = document.getElementById('dcc-recovery');
    const escalate = document.getElementById('dcc-escalate');
    ...
    if (recovery) recovery.onclick = async () => {
      if (typeof window.quickAsk === 'function') {
        await window.quickAsk(`Generate denial recovery plan for ${this.selectedOffice}`);
      }
    };

    if (escalate) escalate.onclick = async () => {
      if (typeof window.quickAsk === 'function') {
        await window.quickAsk(`Draft payer escalation for ${this.selectedOffice}`);
      }
    };
  ...
  startAutoRefresh(){
    ...
    this.timer = setInterval(() => {
      this.load().catch(err => console.error('DEECC auto-refresh failed', err));
    }, 15000);
  }
};

setTimeout(() => {
  window.DEECC.load().then(() => window.DEECC.startAutoRefresh()).catch(err => console.error('DEECC load failed', err));
}, 500);
</script>
"""
...
if "window.DEECC" not in text:
    ...
    # Replace existing DEECC block
    text = re.sub(
        r"<style>\s*#dee-command-center\{[\s\S]*?</style>\s*<script>\s*window\.DEECC\s*=\s*\{[\s\S]*?setTimeout\(\s*\(\)\s*=>\s*\{\s*window\.DEECC\.load\(\)\.then\(\(\)\s*=>\s*window\.DEECC\.startAutoRefresh\(\)\)\.catch\(err => console\.error\('DEECC load failed', err\)\);\s*\},\s*500\s*\);\s*</script>",
        inject,
        text,
        flags=re.S
    )
...
print("Applied Dee Command Center polish")
PY

git add "$FILE"
git commit -m "Polish Dee command center with auto-refresh, drilldown, and scroll fixes" || true
