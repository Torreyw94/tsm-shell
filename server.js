...
app.post('/api/groq', (req, res) => {
  ...
  const groqReq = https.request(options, groqRes => {
    ...
    groqRes.on('end', () => {
      try {
        res.status(groqRes.statusCode || 200).json(JSON.parse(body));
      } catch {
        res.status(groqRes.statusCode || 200).send(body);
      }
    });
  });
...
app.get('/api/ping', async (req, res) => {
  ...
  try {
    ...
    const live = await new Promise(resolve => {
      const r = mod.request(url, { method: 'HEAD', timeout: 5000 }, res2 => resolve((res2.statusCode || 500) < 500));
...
<div class="footer">TSM Matter · tsm-shell.fly.dev · Built with Suite Builder</div>
  ...
  try {
    fs.writeFileSync(path.join(suiteDir, 'index.html'), html, 'utf8');
...
app.put('/api/hc/reports/:id', (req, res) => {
  ...
  writeJson(HC_REPORTS_FILE, reports);
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    reports: readJson(HC_REPORTS_FILE, []).length,
    nodes: Object.keys(readJson(HC_NODE_STATE_FILE, {})).length
  });
});
...
// ── START ────────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`TSM Node API · port ${PORT}`);
});
