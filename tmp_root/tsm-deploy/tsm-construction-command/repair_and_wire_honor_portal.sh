...
# 1) Remove obvious raw diff markers / broken pasted patch lines
cleaned = []
for line in text.splitlines():
    ...
    if stripped.startswith('+'):
        ...
        line = line[:prefix_len] + stripped[1:]
    cleaned.append(line)

text = "\n".join(cleaned)

# 2) Remove any previously injected duplicate honor block/script if present
text = re.sub(
    r'\n?<div id="honor-neural-panel"[\s\S]*?<script>\s*const HONOR_SCOPE = \{[\s\S]*?setInterval\(loadHonorPortalIntel, 15000\);\s*\}\);\s*</script>\n?',
    '\n',
    text,
    count=1
)
...
echo "== SYNTAX CHECK (HTML presence only) =="
grep -n "honor-neural-panel\\|HONOR_SCOPE\\|fetchHonorLayer2\\|fetchHonorBrief" "$FILE"

echo "== COMMIT =="
git add "$FILE"
git commit -m "Repair and wire honor portal to live hc strategist endpoints" || true
