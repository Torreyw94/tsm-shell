...
mkdir -p backups/dee_mode_renderer
cp -f "$DST" "backups/dee_mode_renderer/index.$STAMP.bak"
...
function renderRecovery(data){
  ...
  return `
    <div class="priority">
      ...
      <div class="priority-body">${esc(ab.topPriorityNow || ad.title || 'Run denial recovery')}</div>
    ...
    <div class="grid2" style="margin-top:14px">
      <div class="card">
        <div class="label">${esc(ad.title || 'Recovery Steps')}</div>
        <div class="list">
          ${(ad.steps || l.bestNextActions || []).map(s => `<div class="item"><div class="item-body">${esc(s)}</div></div>`).join('') || '<div class="item"><div class="item-body">No recovery steps returned.</div></div>'}
        </div>
      </div>
      <div class="card">
    ...
    <div class="section" style="margin-top:14px;padding:14px">
      ...
      <div class="narrative">${esc(data.actionBoard?.strategistNarrative || '')}</div>
    </div>

    <div class="section" style="margin-top:14px;padding:14px">
      <div class="section-title">Recovery Alerts</div>
      <div class="stack">${renderAlerts((data.alerts || []).filter(a => ['DENIAL_SPIKE','QUEUE_PRESSURE'].includes(a.type)))}</div>
    </div>
  `;
}
...
function renderPayer(data){
  ...
  return `
    ...
    <div class="section" style="margin-top:14px;padding:14px">
      ...
      <div class="stack">${renderAlerts((data.alerts || []).filter(a => a.type === 'AUTH_DELAY'))}</div>
    </div>
  `;
}
  ...
  return `
    ...
    <div class="grid2" style="margin-top:14px">
      <div class="card">
        ...
        <div class="narrative">${esc(ab.strategistNarrative || 'No executive narrative returned.')}</div>
      </div>
      <div class="card">
...
print("WROTE CLEAN DIFFERENTIATED MODES FILE")
PY

git add "$DST"
git commit -m "Rebuild Dee page with real differentiated modes (safe write)" || true
